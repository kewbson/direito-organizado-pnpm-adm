...
// Adicione esta nova função no final do arquivo src/services/questionsService.js
...
export const getQuestionsStats = async () => {
  try {
    ...
    querySnapshot.forEach((doc) => {
      totalSubjects++; // Cada documento na coleção 'quizzes' é uma matéria
      const data = doc.data();
    ...
    return { success: true, stats: { totalSubjects, totalQuestions } };
  } catch (error) {
